...
/*   Updated: 2025/07/16 17:50:19 by tchow-so         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/render.h"

static void	move_front(t_raycaster *rc, unsigned int *last_move);
static void	move_left(t_raycaster *rc, unsigned int *last_move);
static void	move_back(t_raycaster *rc, unsigned int *last_move);
...
void	calc_player_movement(t_raycaster *rc)
{
	...
	{
		if (rc->key_state[0] == 1)
			move_front(rc, &last_move);
		if (rc->key_state[1] == 1)
			move_left(rc, &last_move);
		if (rc->key_state[2] == 1)
			move_back(rc, &last_move);
		if (rc->key_state[3] == 1)
